...
function SectionHeader({ section, items, selectMode, deleteMode, selected, onGroupToggle, isCoadmin, isAdmin, sortOrder, groupByDate, myUploadsFilter, onMyUploadsToggle, showMyUploadsOnOther, hideMyUploads }: {
  ...
  const subText = isMain
    ? groupByDate
      ? `(grouped by date · ${sortOrder === 'desc' ? 'newest first' : 'oldest first'})`
        ...
        {deleteMode && (
          <View style={styles.sectionSelectRow}>
            ...
            {isCoadmin && !myUploadsFilter && (
              <Text style={styles.deleteNote}>You can delete any guest's photos, but not photos uploaded by the Organiser or other Co-Admins.</Text>
            )}
          </View>
        )}
      </View>
...
export default function EventScreen() {
  ...
  const deletablePhotos = myUploadsFilter && userMobile
    ...
    ? [...photos, ...otherPhotos].filter(p => p.uploaded_by_mobile !== ownerPhone && !coadminPhones.has(p.uploaded_by_mobile))
    : [...photos, ...otherPhotos];
  ...
  const { listData, stickyIndices, orderedMain, orderedOther } = useMemo(() => {
    ...
    const deleteFilteredPhotos = deleteMode && myUploadsFilter && userMobile
      ...
      ? photos.filter(p => p.uploaded_by_mobile === userMobile)
      : deleteMode && isCoadmin
      ? photos.filter(p => p.uploaded_by_mobile !== ownerPhone && !coadminPhones.has(p.uploaded_by_mobile))
      : photos;
    const deleteFilteredOther = deleteMode && myUploadsFilter && userMobile
      ...
      ? otherPhotos.filter(p => p.uploaded_by_mobile === userMobile)
      : deleteMode && isCoadmin
      ? otherPhotos.filter(p => p.uploaded_by_mobile !== ownerPhone && !coadminPhones.has(p.uploaded_by_mobile))
      : otherPhotos;
    ...
    if (orderedMain.length > 0) {
      ...
      if (groupByDate) {
        // Group by date respecting sortOrder: sortOrder=desc → newest date first
        const sourceForGroups = sortOrder === 'desc' ? [...baseMain].reverse() : [...baseMain];
        ...
        for (let i = 0; i < orderedMain.length; i += 3) {
          items.push({ type: 'photo_row', photos: orderedMain.slice(i, i + 3), section: 'main', startIndex: i, key: `row_main_${i}` });
        }
      }
    }
          ...
          <View style={styles.eventHeader}>
            <View style={styles.eventHeaderTopRow}>
              ...
              <TouchableOpacity style={styles.backBtn} onPress={() => {
                ...
                clearLastEvent();
                router.canGoBack() ? router.back() : router.replace('/(auth)/home');
              }}>
          ...
          <View style={{ opacity: stickySection === item.section ? 0 : 1 }}>
            <SectionHeader
              ...
              items={item.section === 'main'
                ? (deleteMode && myUploadsFilter && userMobile ? photos.filter(p => p.uploaded_by_mobile === userMobile) : deleteMode && !isAdmin && userMobile ? photos.filter(p => p.uploaded_by_mobile === userMobile) : deleteMode && isCoadmin ? photos.filter(p => p.uploaded_by_mobile !== ownerPhone && !coadminPhones.has(p.uploaded_by_mobile)) : myUploadsFilter && userMobile ? photos.filter(p => p.uploaded_by_mobile === userMobile) : photos)
                : (deleteMode && myUploadsFilter && userMobile ? otherPhotos.filter(p => p.uploaded_by_mobile === userMobile) : deleteMode && !isAdmin && userMobile ? otherPhotos.filter(p => p.uploaded_by_mobile === userMobile) : deleteMode && isCoadmin ? otherPhotos.filter(p => p.uploaded_by_mobile !== ownerPhone && !coadminPhones.has(p.uploaded_by_mobile)) : myUploadsFilter && userMobile ? otherPhotos.filter(p => p.uploaded_by_mobile === userMobile) : otherPhotos)}
              selectMode={selectMode}
              deleteMode={deleteMode}
              selected={selected}
            ...
            {(['asc', 'desc'] as const).map(mode => (
              <TouchableOpacity key={mode} style={styles.sortPanelOption} onPress={() => setDraftSortOrder(mode)}>
                <View style={[styles.sortRadio, draftSortOrder === mode && styles.sortRadioSelected]}>
                  {draftSortOrder === mode && <View style={styles.sortRadioDot} />}
                </View>
...
const styles = StyleSheet.create({
  ...
  myUploadsPill: { borderWidth: 1, borderColor: '#666', borderRadius: 20, paddingHorizontal: 12, paddingVertical: 5 },
  myUploadsPillActive: { backgroundColor: Colors.accent, borderColor: Colors.accent },
  myUploadsPillText: { fontSize: 12, fontWeight: '600', color: '#fff' },
